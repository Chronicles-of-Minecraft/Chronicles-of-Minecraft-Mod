...
public class AridDesertBiome extends Biome {
    private static SurfaceBuilderConfig surfaceBuilderConfig = new SurfaceBuilderConfig(
            Blocks.SAND.getDefaultState(), Blocks.SAND.getDefaultState(), Blocks.SANDSTONE.getDefaultState());
            
    protected AridDesertBiome() {
        super((new Biome.Builder())
                .surfaceBuilder(new ConfiguredSurfaceBuilder<>(SurfaceBuilder.DEFAULT, surfaceBuilderConfig))
                .precipitation(RainType.NONE).category(Category.NONE).depth(0F).scale(0F).temperature(2F).downfall(0F)
